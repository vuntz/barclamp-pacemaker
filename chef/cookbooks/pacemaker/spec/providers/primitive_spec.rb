...
describe "Chef::Provider::PacemakerPrimitive" do
  before do
    ...
    # Initialize run context, load cookbooks, etc.
  ...
  describe ":create action" do
    ...
    it "should modify the primitive if it already exists" do
      ...
      expect_any_instance_of(Chef::Provider::PacemakerPrimitive).to \
        receive(:get_cib_object_definition).and_return(ra[:config])
    ...
    it "should create a primitive if it doesn't already exist", focus: true do
      # get_cib_object_definition is invoked by load_current_resource
      # and later used to see whether to create or modify.
      expect_any_instance_of(Chef::Provider::PacemakerPrimitive).to \
        receive(:get_cib_object_definition).and_return(nil)

      # Later, the :create action calls cib_object_exists? to check
      # that creation succeeded.
      expect_any_instance_of(Chef::Provider::PacemakerPrimitive).to \
        receive(:cib_object_exists?).and_return(true)
